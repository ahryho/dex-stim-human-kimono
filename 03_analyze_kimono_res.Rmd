...
kimono.net[,.(target, r_squared)] %>% unique() %>% 
  ...
  theme_tufte() +
  labs(title = "Distribution of targets",
       x = "R sq")
...
ggplot(graph.statistics[1:num.vert,], aes(x = 1:num.vert, y = betweennes.vert )) +
  ...
  labs(x = paste0("First ", num.ensg, " predictors"),
       y = "Vertex betweenness")
```


#### Take first 100 ENSG ID for further analysis
...
vertices.unique <- unique(vertices)

# Create graph
g <- graph_from_data_frame(relations, vertices.unique, directed = FALSE)
...
#vertices[edges > num.edge.thr,]
#vertices[,.N, by = edges,][order(edges)]
vertices[edges < num.edge.thr, new_names := NA]

vertices <- merge(vertices, map.ensg.gene.tbl, by.x = "new_names", by.y = "Ensemble_ID", all.x = T)
